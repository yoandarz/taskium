...
public class AlarmReceiver extends BroadcastReceiver {
    ...
    @Override public void onReceive(Context context, Intent intent) {
        ...
        if (body == null) { AlarmScheduler.scheduleAll(context, json); return; }
        createChannel(context);
        Intent full = new Intent(context, AlarmActivity.class).putExtra("body", body).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
    ...
    static String pendingText(String json) {
        try {
            JSONObject state = new JSONObject(json); JSONArray tasks = state.getJSONArray("tasks");
            StringBuilder lines = new StringBuilder(); int total=0;
            LocalDate today = LocalDate.now();
            for(int i=0;i<tasks.length();i++){
                JSONObject t=tasks.getJSONObject(i); String kind=t.optString("taskKind","one_time"); String text=t.optString("text","Tarea");
                if ("one_time".equals(kind)) { total++; lines.append("• ").append(text).append("\n"); continue; }
                LocalDate anchor; try { anchor=LocalDate.parse(t.optString("anchorDate",today.toString())); } catch(Exception e){anchor=today;}
                int interval=Math.max(1,t.optInt("intervalDays",1)); long days=Math.max(0,ChronoUnit.DAYS.between(anchor,today));
                int generated=1+(int)(days/interval); int debt=Math.max(0,generated-Math.max(0,t.optInt("completedCount",0)));
                if(debt>0){ total+=debt; lines.append("• ").append(text).append(" — ").append(debt).append(debt==1?" pendiente":" pendientes").append("\n"); }
            }
            if(total==0)return null;
            return "Tienes "+total+(total==1?" pendiente.":" pendientes.")+"\n\n"+lines;
        } catch(Exception e){ return "Hay tareas pendientes en Taskium."; }
    }
}
